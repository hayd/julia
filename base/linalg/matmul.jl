...
function Ac_mul_B{T<:Union(Complex128,Complex64)}(A::StridedMatrix{T}, B::StridedMatrix{T})
...
function A_mul_Bc{T<:Union(Complex128,Complex64)}(A::StridedMatrix{T}, B::StridedMatrix{T})
...
function symmetrize!(A::StridedMatrix, UL::BlasChar='U')
    n = @assertsquare A
    if UL == 'U'
        for i = 1:(n-1), j = (i+1):n
            A[j,i] = A[i,j]
...
function symmetrize_conj!(A::StridedMatrix, UL::BlasChar='U')
    n = @assertsquare A
    if UL == 'U'
        for i = 1:(n-1), j = (i+1):n
            A[j,i] = conj(A[i,j])
